import { Proposition3 } from '../lib/proposition-types'

interface Proposition3TableProps {
  data: Proposition3[]
  searchTerm: string
}

export function Proposition3Table({ data, searchTerm }: Proposition3TableProps) {
        ...
        <table className="w-full border-collapse text-sm" style={{ tableLayout: 'auto' }}>
          <thead style={{ position: 'sticky', top: 0, zIndex: 10 }}>
            ...
            <tr>
              ...
              <th className="p-2 border border-gray-400 bg-[#F0FFF0] text-gray-900 font-semibold text-left" style={{ minWidth: '300px', fontSize: '11px' }}>Decision Makers (CIO/CTO, Head Store Ops, Digital, VP, Procurement, CFO, CEO)</th>
              ...
              <th className="p-2 border border-gray-400 bg-[#F0FFF0] text-gray-900 font-semibold text-left" style={{ minWidth: '250px', fontSize: '11px' }}>Budget Approach / Tier (Capex/Opex/Hybrid; Enterprise/Mid/Premium)</th>
              ...
              <th className="p-2 border border-gray-400 bg-[#FFE4B5] text-gray-900 font-semibold text-left" style={{ minWidth: '400px', fontSize: '11px' }}>Integration Requirements (Cloud APIs [AWS, Azure, GCP, IBM], ServiceNow, Jira, Finance systems [SAP, Oracle ERP, NetSuite], Monitoring tools [Datadog, Dynatrace, New Relic], CMDB integration, existing FinOps tools)</th>
              <th className="p-2 border border-gray-400 bg-[#FFE4B5] text-gray-900 font-semibold text-left" style={{ minWidth: '350px', fontSize: '11px' }}>Deployment Intensity / Timeline (Quick announcement [2-4 weeks], Pilot [6-12 weeks], Full scale rollout [3-12 months], Cloud CoE-led phased rollout)</th>
              <th className="p-2 border border-gray-400 bg-[#FFE4B5] text-gray-900 font-semibold text-left" style={{ minWidth: '400px', fontSize: '11px' }}>Service Expectations (Implementation, Ongoing cloud monitoring, Rightsizing analysis, ELO license usage mapping, Billing anomaly alerts, Monthly FinOps reporting, Cloud governance models, Training for FinOps maturity levels)</th>
              <th className="p-2 border border-gray-400 bg-[#FFE4B5] text-gray-900 font-semibold text-left" style={{ minWidth: '400px', fontSize: '11px' }}>Other Constraints (Data residency, PCI, Security compliance [GDPR, HIPAA, ISO27001], Vendor lock-in concerns, Audit - Microsoft/Oracle/SAP/IBM complexity [multi-AD/fragmented])</th>
              <th className="p-2 border border-gray-400 bg-[#E6E6FA] text-gray-900 font-semibold text-left" style={{ minWidth: '250px', fontSize: '11px' }}>Customer Benchmarking Summary (Potential Customers/Peer Group)</th>
              <th className="p-2 border border-gray-400 bg-[#E6E6FA] text-gray-900 font-semibold text-left" style={{ minWidth: '250px', fontSize: '11px' }}>Additional Comments / Notes by CMI Team</th>
            </tr>
          </thead>
          <tbody>
            {filteredData.map((item, index) => (
              <tr key={item.S_No} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                ...
                <td className="p-2 border border-gray-300 text-gray-800">{item.Deployment_Intensity_Timeline}</td>
                <td className="p-2 border border-gray-300 text-gray-800">{item.Service_Expectations}</td>
                <td className="p-2 border border-gray-300 text-gray-800">{item.Other_Constraints}</td>
                <td className="p-2 border border-gray-300 text-gray-800">{item.Customer_Benchmarking_Summary}</td>
                <td className="p-2 border border-gray-300 text-gray-800">{item.Additional_Comments_Notes_CMI_Team}</td>
              </tr>
            ))}
          </tbody>
        </table>
